...
def local_loggerSetup(use_filename=False):
    ...
    # Formatter
    datefmt='%d-%b-%Y %I:%M %p'
    console_formatter = logging.Formatter( f"%(asctime)s [%(levelname)s] : %(message)s", datefmt=datefmt)
    console_handler.setFormatter(console_formatter)

    formatter = logging.Formatter( f"%(asctime)s [%(levelname)s] (%(module)s): %(message)s", datefmt=datefmt)
